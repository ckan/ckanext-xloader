...
class xloaderCommand(cli.CkanCommand):
    ...
    def command(self):
        ...
        if self.args[0] == 'submit':
            ...
            if self.args[1] == 'all':
                ...
                util.submit_all(print)
            elif self.args[1] == 'all-existing':
                self._confirm_or_abort()
                self._load_config()
                self._setup_xloader_logger()
                util.submit_all_existing(print_func=print,
                                         dry_run=self.options.dry_run)
            else:
                pkg_name_or_id = self.args[1]
                self._load_config()
                self._setup_xloader_logger()
                util.submit_package(pkg_name_or_id, print_func=print)
        elif self.args[0] == 'status':
            self._load_config()
            util.printable_status(print_func=print)
        else:
    ...
    def _setup_xloader_logger(self):
        ...
        # cli, let's ensure xloader debug messages are printed for the user
