...
def mock_actions(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            with mock.patch(
                "ckanext.xloader.jobs.get_action"
            ) as mock_get_action:
...
class TestxloaderDataIntoDatastore(object):
    ...
    def setup_class(self):
        ...
        conn.execute('DROP TABLE IF EXISTS "{}"'.format(self.resource_id))
        yield
        if "_datastore" in dir(self):
            connection = self._datastore[1]
            connection.close()

    def register_urls(
        self, filename="simple.csv", content_type="application/csv"
    ):
    ...
    def get_datastore_table(self):
        ...
        return dict(
            ...
            header_dict=OrderedDict(
                [(c.key, six.text_type(c.type)) for c in table.columns]
            ),
    ...
    def get_load_logs(self, task_id):
        ...
        result = conn.execute(
            select([logs.c.level, logs.c.message]).where(
                logs.c.job_id == task_id
            )
        )
    ...
    def test_simple_csv(self):
        ...
        with mock.patch(
            "ckanext.xloader.jobs.set_resource_metadata"
...
class Logs(list):
    def get_errors(self):
        return [message for level, message in self if level == "ERROR"]

    def grep(self, text):
        return [message for level, message in self if text in message]
